...
echo ""
read -p "$( echo -e "Press ${RED}[ ${NC}${green}Enter${NC} ${RED}]${NC} For Starting Installation") "
echo ""
clear
if [ "${EUID}" -ne 0 ]; then
...
# USERNAME
rm -f /usr/bin/user
username=$(curl -sS https://raw.githubusercontent.com/ryuvogs/regip/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl -sS https://raw.githubusercontent.com/ryuvogs/regip/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl -sS https://raw.githubusercontent.com/ryuvogs/regip/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/ryuvogs/v3/main/"
...
# Buat direktori xray
print_install "Installed direktori xray"
    mkdir -p /etc/xray
...
function base_package() {
    clear
    ########
    print_install "Installed Required Packages"
...
#install cf
wget ${REPO}root/cf.sh && chmod +x cf.sh && ./cf.sh
rm -f /root/cf.sh
clear
else
...
function notif_bot() {
    USRSC=$(curl https://raw.githubusercontent.com/ryuvogs/regip/main/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(curl https://raw.githubusercontent.com/ryuvogs/regip/main/ip | grep $ipsaya | awk '{print $3}')
...
function pasang_ssl() {
clear
print_install "Installed SSL Pada Domain"
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version 1.7.5
    ...
    wget -O /etc/xray/config.json "${REPO}root/config.json" >/dev/null 2>&1
    wget -O /etc/systemd/system/runn.service "${REPO}root/runn.service" >/dev/null 2>&1
    ...
    curl -s ipinfo.io/city >>/etc/xray/city
    curl -s ipinfo.io/org | cut -d " " -f 2-10 >>/etc/xray/isp
    print_install "Installed Konfigurasi Packet"
    wget -O /etc/haproxy/haproxy.cfg "${REPO}root/haproxy.cfg" >/dev/null 2>&1
    wget -O /etc/nginx/conf.d/xray.conf "${REPO}root/xray.conf" >/dev/null 2>&1
    sed -i "s/xxx/${domain}/g" /etc/haproxy/haproxy.cfg
    sed -i "s/xxx/${domain}/g" /etc/nginx/conf.d/xray.conf
    sed -i "s/xxx/${ipsaya}/g" /etc/squid/squid.conf
    curl ${REPO}root/nginx.conf > /etc/nginx/nginx.conf
...
function ssh(){
clear
print_install "Installed Password SSH"
    wget -O /etc/pam.d/common-password "${REPO}root/password"
...
function udp_mini(){
clear
print_install "Installed Service Limit"
wget ${REPO}root/limit.sh && chmod +x limit.sh && ./limit.sh
cd
wget -q -O /usr/local/sbin/limit-all-ip "${REPO}root/limit-all-ip"
...
wget -q -O /usr/local/kyt/udp-mini "${REPO}root/udp-mini"
chmod +x /usr/local/kyt/udp-mini
wget -q -O /etc/systemd/system/udp-mini-1.service "${REPO}root/udp-mini-1.service"
wget -q -O /etc/systemd/system/udp-mini-2.service "${REPO}root/udp-mini-2.service"
wget -q -O /etc/systemd/system/udp-mini-3.service "${REPO}root/udp-mini-3.service"
...
wget -q -O /etc/ssh/sshd_config "${REPO}root/sshd" >/dev/null 2>&1
...
clear
print_install "Installed Dropbear"
# // Installing Dropbear
#apt-get install dropbear -y > /dev/null 2>&1
wget -q -O /etc/default/dropbear "${REPO}root/dropbear" >/dev/null 2>&1
...
function ins_vnstat(){
clear
print_install "Installed Vnstat"
# setting vnstat
...
clear
print_install "Installed OpenVPN"
#OpenVPN
wget ${REPO}root/openvpn &&  chmod +x openvpn && ./openvpn
...
function ins_backup(){
clear
print_install "Installed Backup Server"
#BackupOption
apt install rclone -y
printf "q\n" | rclone config
wget -O /root/.config/rclone/rclone.conf "${REPO}root/rclone.conf"
...
wget -q -O /etc/ipserver "${REPO}root/ipserver" && bash /etc/ipserver
...
function ins_swab(){
clear
print_install "Installed Swap 1 GB"
...
clear
print_install "Installed Fail2ban"
...
# Ganti Banner
wget -O /etc/kyt.txt "${REPO}root/issue.net"
print_success "Fail2ban"
}

function ins_epro(){
clear
print_install "Installed ePro WebSocket Proxy"
    wget -O /usr/bin/ws "${REPO}root/ws" >/dev/null 2>&1
    wget -O /usr/bin/tun.conf "${REPO}root/tun.conf" >/dev/null 2>&1
    wget -O /etc/systemd/system/ws.service "${REPO}root/ws.service" >/dev/null 2>&1
...
wget -O /usr/sbin/ftvpn "${REPO}root/ftvpn" >/dev/null 2>&1
...
function menu(){
    clear
    print_install "Installed Menu Packet"
    wget ${REPO}root/menu.zip
    unzip menu.zip
    chmod +x menu/*
...
function noobz(){
    clear
    print_install "Installed NobbzvpnS"
    wget ${REPO}root/noobzvpns.zip
    unzip noobzvpns.zip
	bash install.sh
	rm noobzvpns.zip
...
clear
print_install "Enable Service Script"
    systemctl daemon-reload
    systemctl start netfilter-persistent
    systemctl enable --now rc-local
    systemctl enable --now cron
